	...
	private void translateSurface(Element element, IDFFileObject file) {
		...
		if (surfaceType.contains("Shade")) {
			...
			if (adjacentSpaceElements.isEmpty()) {
				...
				spaceId1 = adjacentSpaceElements.get(0).getAttributeValue("spaceIdRef");
				spaceId2 = adjacentSpaceElements.get(1).getAttributeValue("spaceIdRef");
...
//				 idfWriter.recordInputs(surfaceName + "_reversed", "", "Outside Boundary Condition Object", ""); //25-Oct: original
				idfWriter.recordInputs(surfaceName, "", "Outside Boundary Condition Object", "");  //25-Oct
//				idfWriter.recordInputs("", "", "Outside Boundary Condition Object", "");  //25-Oct
...
//						envelopeTranslator.setReversedConstruction(constructionIdRef + "_reversed", reversedConsName);
                ...
                if (eplusSurfaceType=="Ceiling") {eplusSurfaceTypeReverse="Floor";}
                if (eplusSurfaceType=="Floor") {eplusSurfaceTypeReverse="Ceiling";}
                if (eplusSurfaceType=="Wall") {eplusSurfaceTypeReverse="Wall";}
                if (eplusSurfaceType=="Roof") {eplusSurfaceTypeReverse="Roof";}
...
//				idfWriter.recordInputs(reversedConsName, "", "Construction Name", "");//31-Oct:Origin
				idfWriter.recordInputs(space2Name, "", "Zone Name", "");
				idfWriter.recordInputs(outsideBoundaryCondition, "", "Outside Boundary Condition", "");
//				idfWriter.recordInputs(surfaceName, "", "Outside Boundary Condition Object", ""); //25-0ct:
				idfWriter.recordInputs(surfaceName + "_reversed", "", "Outside Boundary Condition Object", ""); //25-0ct: 
...
//			openingType = "Window";
		boolean flgSkyLight = false; // Addition
		if (openingType.contains("FixedWindow") || openingType.contains("OperableWindow")) { 
			openingType = "Window";
...
//			} else if (openingType.contains("FixedSkylight") || openingType.contains("OperableSkylight")) {			
		...
		if (coordinateList.size() == 4 || coordinateList.size() == 3) {  //original 25-0ct	
			...
			idfWriter.recordInputs(constructionName, "", "Construction Name", "");
//			idfWriter.recordInputs("", "", "Building Surface Name", "");
...
//			idfWriter.recordInputs(subSurfaceName, "", "Outside Boundary Condition Object", "");  //25-0ct
			...
			for (int i = 0; i < coordinateList.size() - 2; i++) {
				...
				idfWriter.recordInputs(openingType, "", "Surface Type", "");
				idfWriter.recordInputs(constructionName, "", "Construction Name", "");
//				idfWriter.recordInputs("", "", "Building Surface Name", "");  //25-0ct
				idfWriter.recordInputs(hostSurfaceName, "", "Building Surface Name", "");  
//				idfWriter.recordInputs(subSurfaceName, "", "Outside Boundary Condition Object", "");  //25-0ct
				idfWriter.recordInputs("", "", "Outside Boundary Condition Object", "");  //25-0ct: Original
				idfWriter.recordInputs("autocalculate", "", "View Factor to Ground", ""); 
	...
	private void convertThermalZone(String spaceName, String spaceType, GbXMLThermalZone thermalZone,
		...
		if (thermalZone != null) {
			...
			String thermostatName = "Dual Setpoint Dual SP"+spaceName;
...
//			comments.add(comment[i]);
			comments.add(comment[i+1]); //revise
		}
